...
|-----------|-------------|
| InceptionBase | Shared InceptionV3 feature extractor (aux logits disabled). Two instances — one for dermoscopy, one for clinical images. |
| DiagnosisMultimodalNet | Dual-backbone image branch. Produces per-image logits (out_d, out_c), combined image features (img_feat), and GAP vectors. |
| FTTransformerEncoder | Converts tabular features into tokens and processes them using a Transformer to produce tabular features (tab_feat). |
| CrossModalAttentionFusion | Projects image and tabular features, applies attention, and combines them into a fused representation. |
| TripleCLIPHead | Aligns image and tabular features in a shared embedding space using contrastive learning. |
| ReliabilityHead | Used in Phase 4. Predicts modality reliability and outputs weights (gate_w_img, gate_w_tab). |
| FragilityAwareSampler | Dynamically reweights samples based on model confusion (fragility score) to focus training on hard cases. |
| TwoSidedGradientBalancing | Adjusts loss weights using gradient ratios to keep image and tabular learning balanced. |
